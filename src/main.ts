...
loadClamps();
updateClamps();
document.getElementById('applyOptions')?.addEventListener('click', async () => {
    // preserve simulation state
    const snapshot = await game.instance.saveSnapshot();
    game.instance.destroy();
    game.instance = new WGPUSoftbodyEngine(canvas, resolution, game.options);
    game.instance.setPhysicsConstants(game.constants);
    await game.instance.loadSnapshot(snapshot);
});
